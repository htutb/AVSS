...
class DPTNTransformer(nn.Module):
    ...
    '''
    def __init__(self, embed_dim: int, nhead: int, dropout: float, lstm_dim: int, bidirectional: bool):
        super().__init__()

        self.attention = nn.MultiheadAttention(embed_dim=embed_dim, num_heads=nhead, dropout=dropout, batch_first=True)

        self.norm1 = nn.LayerNorm(embed_dim)
        self.norm2 = nn.LayerNorm(embed_dim)
        self.ffn_act = nn.ReLU()

        self.lstm = nn.LSTM(input_size=embed_dim, hidden_size=lstm_dim, batch_first=True, bidirectional=bidirectional)
...
class DPTNBlock(nn.Module):
    ...
    '''
    def __init__(self, feature_dim: int, nhead: int, dropout: float, lstm_dim: int, bidirectional: bool):
        super().__init__()

        self.intra_transformer = DPTNTransformer(feature_dim, nhead, dropout, lstm_dim, bidirectional)
        self.inter_transformer = DPTNTransformer(feature_dim, nhead, dropout, lstm_dim, bidirectional=False)
...
class MaskCreator(nn.Module):
    ...
    def __init__(self, feature_dim: int, N: int, C: int):
        ...
        self.tanh = nn.Tanh()
        self.tanh_conv = nn.Conv1d(in_channels= feature_dim, out_channels= feature_dim, kernel_size=1, bias=False)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x_sigmoid = self.sigmoid(self.sigmoid_conv(x)) # [batch * C, feature_dim, T_new]
...
class DPTNSeparator(nn.Module):
    ...
    def __init__(self, feature_dim: int, K: int, H: int, nhead: int, dropout: float, lstm_dim: int, bidirectional: bool, R: int, N: int, C: int):
        super().__init__()
        self.C = C
        self.N = N
        self.feature_dim = feature_dim

        self.feat_conv = nn.Conv1d(in_channels=N, out_channels=feature_dim, kernel_size=1, bias=False)
        self.segmentation = Segmentation(K, H)
        self.global_norm = GlobalLayerNorm(N)

        self.dptn_blocks = nn.ModuleList([DPTNBlock(feature_dim, nhead, dropout, lstm_dim, bidirectional) for _ in range(R)])

        self.act = nn.PReLU()
        self.conv2d = nn.Conv2d(in_channels=feature_dim, out_channels=feature_dim*C, kernel_size=1, bias=False)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x.view(B * self.C, self.feature_dim, K, num_chunks)  # [B*C, feature_dim, K, num_chunks]
