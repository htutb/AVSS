...
def collate_fn(dataset_items: list[dict]):
    ...
    s1_audios = torch.tensor([item["s1_audio"] for item in dataset_items])
    s2_audios = torch.tensor([item["s2_audio"] for item in dataset_items])
    mix_audios = torch.tensor([item["mix_audio"] for item in dataset_items])

    s1_mouths = torch.tensor([item["mouth1"] for item in dataset_items])
    s2_mouths = torch.tensor([item["mouth2"] for item in dataset_items])  
